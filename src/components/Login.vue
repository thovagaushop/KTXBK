...
<div class="body-login">
  <div class="container">
		...
		<div class="cover">
            ...
            &nbsp;&nbsp;
						<input type="email" placeholder="Nhập email" v-model="email" required>
					</div>
					<div class="input-box">
            ...
            &nbsp;&nbsp;
					...
					<div class="text login-text">Bạn chưa có tài khoản? <router-link :to="{name: 'SignUp'}">Đăng ký tại đây</router-link></div>
				</div>
			</div>
		</div>
		</form>	
        ...
        loginSubmit() {
            ...
            axios.post('/api/login', formData).then(resp => {
                ...
                axios.get('/api/auth', {
                    ...
                    if (user.is_admin) this.$router.push('/admin')
                    else if (user.is_cadres) this.$router.push('/cardres')
                ...
                if (err.response.data.non_field_errors.length) {
                    console.log(err.response)
                    toast({
                    message: "You have login wrong account!!!",
                    type: 'is-danger',
                    dismissible: true,
                    pauseOnHover: true,
                    duration: 10000,
